...
//            "1.0"
    ...
    fun geometricMean(lines: List<Line>): Pair<Double, Double> {
        return Pair(geometricMean(lines.map { it.buySlippage }), geometricMean(lines.map { it.sellSlippage }))
    }
    ...
    val coinToSlippages = lines.groupBy { it.coinAndAmount }.mapValues { geometricMean(it.value) }
    coinToSlippages.forEach { coin, slippages ->
        var needPrint = false
        amounts.forEach {
            if (coin.endsWith(it)) needPrint = true
        }
        if (needPrint)
            resultFile.appendText("$coin\t${slippages.first}\t${slippages.second}\n")
    }
}