...
class TestDatabase private constructor(private val connection: Connection) : AutoCloseable {
    ...
    // todo при вставке, база блокируется, пока все свечи не будут добавлены в базу. но в тоже время в candles записываются новые свечи из сети, и все они хранятся в памяти
    ...
    companion object {
        suspend fun create(path: Path): TestDatabase {
            Class.forName("org.sqlite.JDBC")
            val connection = DriverManager.getConnection("jdbc:sqlite:$path")
            connection.autoCommit = false
            return TestDatabase(connection).apply {
                update()
            }
        }
    }
